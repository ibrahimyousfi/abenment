<x-gym-layout>
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Modifier le type d'abonnement</h1>
                <p class="text-gray-500 text-sm mt-1">Modification : {{ $trainingType->name }}</p>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
            <div class="p-8">
                <form method="POST" action="{{ route('gym.training-types.update', $trainingType) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $trainingType->name) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out sm:text-sm bg-gray-50" required>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out sm:text-sm bg-gray-50">{{ old('description', $trainingType->description) }}</textarea>
