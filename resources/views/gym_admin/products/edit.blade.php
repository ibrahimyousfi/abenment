    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Modification : {{ $product->name }}</p>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
            <div class="p-8">
                <form method="POST" action="{{ route('gym.products.update', $product) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out sm:text-sm bg-gray-50" required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="relative rounded-md shadow-sm">
                                <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" class="block w-full rounded-lg border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50" required>
                        ...
                        <!-- Stock -->
                        <div class="mb-6">
                            <label for="stock" class="block text-sm font-bold text-gray-700 mb-2">Stock actuel</label>
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" min="0" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out sm:text-sm bg-gray-50" required>
                    ...
                    @if($product->image_path)
                        <div class="mb-6">
                            ...
                            <div class="mt-2">
                                <img src="{{ asset('storage/' . $product->image_path) }}" alt="{{ $product->name }}" class="h-32 w-32 object-cover rounded-lg border border-gray-200">
                            </div>
                        </div>
                    @endif
                    ...
                    <!-- New Image -->
                    <div class="mb-6">
                        <label for="image" class="block text-sm font-bold text-gray-700 mb-2">Changer l'image (Optionnel)</label>
